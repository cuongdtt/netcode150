const {
  minimumRecolors,
} = require("./2379_minimum_recolors_to_get_k_consecutive_black_blocks.js");
...
describe("Minimum Recolors to Get K Consecutive Black Blocks - Common Patterns", () => {
  test("should handle alternating pattern", () => {
    ...
    expect(minimumRecolors("WBWBWB", 3)).toBe(1);
  });
  ...
  test("should handle blocks with consecutive whites", () => {
    expect(minimumRecolors("BWWWB", 3)).toBe(2);
    expect(minimumRecolors("BWWWB", 2)).toBe(1);
  });
...
describe("Minimum Recolors to Get K Consecutive Black Blocks - Complex Scenarios", () => {
  test("should handle multiple possible windows", () => {
    ...
    expect(minimumRecolors("BWBWBWB", 3)).toBe(1);
  });
  ...
  test("should handle scattered black blocks", () => {
    expect(minimumRecolors("BWBWBWB", 4)).toBe(2);
    expect(minimumRecolors("BWWBWWB", 5)).toBe(3);
  });
...
describe("Wrong Cases", () => {
  test("should handle random cases", () => {
    expect(minimumRecolors("WBWBBBW", 2)).toBe(0);
  });
});
